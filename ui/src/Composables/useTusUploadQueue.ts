...
import { getCsrfToken, niceSize } from '@modufolio/panel'

/**
 * Generic, domain-free TUS resumable-upload queue (extraction-ready; future
 * `@modufolio/panel-uploads`). Knows nothing about media libraries — apps
 * hook domain behavior via the options callbacks. The media-specific wrapper
 * is `useTusUpload` (app-land), which layers the library listing on top.
 */
...
export function useTusUploadQueue(options: UseTusUploadQueueOptions) {
  ...
  /** @deprecated Import {@link niceSize} from @modufolio/panel instead. */
