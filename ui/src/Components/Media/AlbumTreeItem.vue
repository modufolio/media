  ...
  <div>
    ...
    <div
      :class="[
        ...
        isSelected
          ? 'bg-pressed text-ink font-medium'
          : 'text-ink hover:bg-hover focus-visible:bg-hover',
        isDragOver
          ? 'ring-2 ring-line-strong bg-hover'
          : '',
      ]"
    ...
    >
      ...
      <span
        class="absolute top-1/2 -translate-y-1/2 pointer-events-none flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-grab text-ink-3 transition-opacity"
      ...
      <button
        ...
        class="absolute top-1/2 -translate-y-1/2 flex items-center justify-center w-3.5 h-4 text-ink hover:text-ink-2"
      ...
      <span class="flex items-center gap-2 min-w-0 flex-1">
        <svg v-if="album.album_type === 1" class="w-4 h-4 flex-shrink-0 text-ink" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
        </svg>
        <svg v-else class="w-4 h-4 flex-shrink-0 text-ink" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Media count -->
      <span class="text-xs text-ink-3 flex-shrink-0">{{ albumCounts[album.id] ?? album.media_count }}</span>
    </div>
    ...
    <div v-if="isExpanded && localChildren.length > 0">
      <template v-for="(child, index) in localChildren" :key="child.id">
        ...
        <div
          ...
          class="h-0.5 mr-3 rounded-full bg-primary pointer-events-none"
        />
